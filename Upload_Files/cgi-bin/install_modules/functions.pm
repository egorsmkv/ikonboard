...
sub database {
    ...
    if ( $iB::IN{'DO'} ) {
        ...
        elsif ($iB::IN{'DB_DRIVER'} eq 'Oracle') {
            $html = $start->setup_oracle();
        }
...
sub system_test {
    ...
    my ($obj, $page_title, $magick) = @_;

      eval { require Image::Magick };
           $magick    = $@ ? 'No' : 'Yes';

    my $recommend = "A <b>DBM</b> database will be sufficient for small boards 
    				(less than 100 users/1000 posts).";


    my $has_dbm      = $iB::EVAL->{'DB_File'};
    my $has_cgi      = $iB::EVAL->{'CGI'};
    my $has_dbi      = $iB::EVAL->{'DBI'};
    #my $has_magick   = $iB::EVAL->{'IMAGE::MAGICK'};
    ...
    if ($has_dbi) {
        $recommend .= "<br><br>For system and database safety, we recommend that you ask your webhost 
        				about mySQL. Your perl installation supports mySQL. If you have access to a 
        				mySQL database, then we suggest you use the <b>mySQL</b> option.  This may prevent 
        				errors and problems in the future.";
    }
    ...
    my $html = qq~<table cellspacing='0' cellpadding='4' width='100%' align='center' border='0'>
                  ...
                  <tr>
                   <td class='t'><b>Can use HRI Registration Key (Image::Magick module available)?</b><br>(<i>optional</i>)</td>
                   <td class='t'>$magick</td>
                  </tr>
                  <tr>
...
sub write_boardinfo {
    ...
    my $file = $data->{'IKON_DIR'}.'/Data/Boardinfo.cgi';
