...
sub do_tar {
    ...
    elsif ($tar_name eq 'Database.tar' and $iB::CONFIG->{tmp_RM_DB} eq 'n') {
    goto SKIP;
	}
    elsif ($iB::CONFIG->{tmp_REMOVE} eq 'y' and $tar_name ne 'Database.tar') {
        rmtree "$to/$this_dir";
        mkdir ("$to/$this_dir", 0777);
    }    
...
sub start_tar {
    ...
    return qq~
              ...
              <table cellspacing='0' cellpadding='4' width='100%' align='center' border='0'>
              <tr>
               <td class='ti' colspan='2'><u><b>Having Trouble Installing Ikonboard?</b></u><br><br>You can receive help on our <a href="http://forums.ikonboard.com"><font color="#FF0000">Support Forums</font></a>.  Ask questions on our community forums or submit a trouble ticket.</td>
              </tr>
              <tr>
              ...
              <tr>
               <td class='t'><b>Remove any old databases (members and posts)? This is for DBM users only. If your using SQL select yes. <br>If you select no, you will need to replace manually all the files contained in the /Database/config/ directory.<br> You will also need to create two new empty directories named <i>topic_views</i> and <i>member_notepads</i> located in the /Databese directory.</b></td>
               <td class='t'><select name='RM_DB' class='forminput'><option value='y' selected>Yes</option><option value='n'>No</option></select></td>
              </tr>
              <tr>
                <td align='left' bgcolor='#8888AA'><input type='button' class='forminput' style='width:100' value=' << BACK ' onClick='window.location="installer.$iB::EXT?act=tar&DO=0"'></td>
                <td align='right' bgcolor='#8888AA'><input type='submit' class='forminput' style='width:100' value=' PROCEED >> '></td>
