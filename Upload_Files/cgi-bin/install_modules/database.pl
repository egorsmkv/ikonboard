...
sub setup_mysql {
    ...
    return qq~
          ...
          <tr>
           <td class='t'><b>Allow Ikonboard to create the needed tables?</b><br>You may choose 'No' and use phpMyAdmin to create the tables.<br>Use "/INSTALL_DATA/mysql_schema.txt" as your guide.</td>
...
####################################################
# SET UP Oracle; Oracle Driver By Andrey Prokopenko
####################################################
...
sub setup_oracle {
    ...
    return qq~
          <form action='installer.$iB::EXT' method='post'>
          <input type='hidden' name='act' value='database'>
          <input type='hidden' name='DB_DRIVER' value='Oracle'>
          <input type='hidden' name='DO'    value='1'>
          <input type='hidden' name='create'  value='1'>
          <table cellspacing='0' cellpadding='4' width='100%' align='center' border='0'>
          <tr>
           <td class='ti' colspan='2'><span style='color:#CC0000'><u>Oracle Set Up Information</u></span></td>
          </tr>
          <tr>
           <td class='t' colspan='2'>NOTE: Oracle support has NOT been tested and is provided as-is.  Most likely you will make some changes to the code for it to work.</td>
          </tr>
          <tr>
           <td class='t' colspan='2'>Before we can go any further, we need to know a few details about your Oracle set-up. Oracle must be set up properly for Ikonboard to use it. If you are unsure of any of the requested information, please check wth your web host.</td>
          </tr>
          <tr>
           <td class='t'><b>Your mySQL Username</b> [ Required ]</td>
           <td class='t'><input type='text' class='forminput' name='DB_USER' value='$iB::CONFIG->{'DB_USER'}'></td>
          ...
          <tr>
           ...
           <td class='t'><select name='CREATE' class='forminput'><option value='y' selected>Yes</option><option value='n'>No, I hav$
          </tr>
          <tr>
            <td align='left' bgcolor='#8888AA'><input type='button' class='forminput' style='width:100' value=' << BACK ' onClick='window.location="installer.$iB::EXT?act=database&DO=0"'></td>
            <td align='right' bgcolor='#8888AA'><input type='submit' class='forminput' style='width:100' value=' PROCEED >> '></td>
...
sub _create_oracle {
    ...
    unless ($found) {
        ++$errors;
        push @fatal_errors, "You must install the Oracle DBD driver.";
        $have_dbd = '<span style="color:red">NO</span>';
    } else {
        $have_dbd = 'Yes';
    }
    ...
    # Test for connection
    $found = 0;
    my $dsn = "DBI:Oracle:(description=(address=(host=$iB::IN{'DB_IP'})(protocol=tcp)(port=1521))(connect_data=(sid=$iB::IN{'DB_NAME'})))";
       $dsn = "DBI:Oracle:(description=(address=(host=$iB::IN{'DB_IP'})(protocol=tcp)(port=$iB::IN{'DB_PORT'}))(connect_data=(sid=$iB::IN{'DB_NAME'})))" if $iB::IN{'DB_PORT'};
    my $dbh = DBI->connect($dsn, $iB::IN{'DB_USER'}, $iB::IN{'DB_PASS'});

    if ($DBI::errstr) {
        ++$errors;
        push @fatal_errors, "Oracle connection error: $DBI::errstr";
        goto 'ERRORS';
    }
    ...
    if ($iB::IN{'CREATE'} eq 'y') {
        local $/ = undef;
        open (ORACLE, "$iB::CONFIG->{'IKON_DIR'}/INSTALL_DATA/oracle_schema.txt") || &iB::install_error( "Cannot locate the INSTALL_DATA/create_tables.txt file");
        @SQLS = split /\;\n/, <ORACLE>;
        close (ORACLE);
...
sub end_setup {
    ...
    return qq~
                  ...
                  <tr>
                   <td class='t' colspan='2'>The database set up procedure is now complete.<br> If you requested to keep your old dastabase you can now stop here before the database info gets overwriten with the base information. If this is an upgrade you can stop here.<br><br>You may now proceed with the next step if it's a new install.</td>
                  </tr>
                  <tr>
                    <td align='left' bgcolor='#8888AA'><input type='button' class='forminput' style='width:100' value=' << BACK ' onClick='window.location="installer.$iB::EXT?act=database&DO=0"'></td>
                    <td align='right' bgcolor='#8888AA'><input type='submit' class='forminput' style='width:100' value=' PROCEED >> '></td>
